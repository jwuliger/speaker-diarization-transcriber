...
This script performs speaker diarization on an audio file using Google Cloud Speech-to-Text API.
...
def perform_diarization(speech_file):
    """
    Perform speaker diarization on the given audio file.

    This function converts the audio to mono if necessary, uploads the audio file 
    ...
    sample_rate, channels = get_wav_info(speech_file)
    ...
    if channels > 1:
        ...
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temp_file:
            mono_file = convert_to_mono(speech_file, temp_file.name)
    ...
    try:
        ...
        formatted_transcription = format_transcription(all_words)
...
if __name__ == "__main__":
    ...
    # Perform diarization
    result = perform_diarization(speech_file)
